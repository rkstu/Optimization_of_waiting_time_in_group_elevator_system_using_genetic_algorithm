...
int main()
{
  ...
  // Collecting data  for Lift
  int nLift;
  cout << "Enter no. of lift : ";
  cin >> nLift;
  Lift car[nLift];
  for (int i = 0; i < nLift; i++)
  {
    ...
    cin >> dir;
    cout << "Enter curr Floor : ";
    cin >> curr_f;
    cout << "Enter no. of des floors : ";
    cin >> nDes;
    cout << "Enter " << nDes << " floors : ";
    vector<int> temp_des;
    for (int j = 0; j < nDes; j++)
    {
      int temp_data;
      cin >> temp_data;
      temp_des.push_back(temp_data);
    }
    car[i].set(dir, curr_f, temp_des);
  }

  // Colecting data for Hall Cals
  int nReq;
  cout << "\nEnter number of hall Calls : ";
  cin >> nReq;
  Request req[nReq];

  for (int i = 0; i < nReq; i++)
  {
    cout << "\n For Hall Call " << i + 1 << endl;
    int dir, des_f, curr_f;
    cout << "Enter direction(0 for Down and 1 for Up) : ";
    cin >> dir;
    cout << "Enter curr floor : ";
    cin >> curr_f;
    cout << "Enter des floor : ";
    cin >> des_f;
    req[i].set(dir, curr_f, des_f);
  }
